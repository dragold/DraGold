...
export function HomePage({ home, onOpenSearch, onOpenCard, onOpenSet, onNavCollection }) {
  ...
  // The full timeline can be 150+ sets — window it to a readable run around
  // the featured card's set (or the most recent releases if it isn't dated).
  ...
  return (
    <div className="home">
      ...
      <Stratum edgeLabel="SET → RELEASE TIMELINE" depth="04" voice="It arrived at a moment." index={0} id="stratum-history" card={featured}>
        {tlWindow.length > 1 ? (
          <DoorRail label={`${TCG_LABEL[featured?.tcg] || "TCG"} sets in release order`}>
            ...
            {tlEnd < timeline.length && (
              <span className="tl-ellipsis">{timeline.length - tlEnd} later →</span>
            )}
          </DoorRail>
        ) : (
        ...
        {currentIdx >= 0 && timeline.length > 1 && (
          <p className="tl-caption">
            {set?.set_name} is release {currentIdx + 1} of {timeline.length} in{" "}
            {TCG_LABEL[featured?.tcg]}{featured?.lang ? ` ${featured.lang.toUpperCase()}` : ""}.
          </p>
        )}
        <Doors>
      ...
      <Stratum
        ...
        voice={
          <>
            Don't just collect it. <span className="threshold-em">Know it.</span>
          </>
        }
        index={0}
        ...
        {typeof setOwned === "number" && typeof setTotal === "number" && setTotal > 0 ? (
          <div className="coll">
            ...
            <p className="coll-copy">
              Gold where you've been, ink where you haven't. Open your collection
              to see every set laid out the same way.
            </p>
          </div>
        ) : (
          <div className="coll">
            ...
            <div className="coll-copy">
              <p>
                ...
                gaps stay dark — a set you're close to finishing is obvious at a
                glance.
              </p>
