...
function ebayURL(name,setName,country,grade=null,tcg="pokemon",cardNumber=""){
  ...
  // Filtra set code interni (bw8, dp7…) — non cercabili su eBay dai compratori
  const safeSet=_isInternalSetCode(setName)?"":(setName?` ${setName}`:"");
  let q;
  if(grade) q=`${name}${num}${safeSet} PSA ${grade} pokemon card`.replace(/\s+/g," ").trim();
  else if(tcg==="mtg") q=`${name}${num}${safeSet} magic the gathering card`.replace(/\s+/g," ").trim();
  else if(tcg==="ygo") q=`${name}${num}${safeSet} yugioh card`.replace(/\s+/g," ").trim();
  else if(tcg==="onepiece") q=`${name}${num}${safeSet} one piece card game`.replace(/\s+/g," ").trim();
  else q=`${name}${num}${safeSet} pokemon card`.replace(/\s+/g," ").trim();
  ...
  return(
    <div ref={ref} className={`holocard${big?' big':''}${small?' sm':''}`}
      ...
      <div className="holocard-tilt">
        {src?<img src={src} alt={alt||''} loading="lazy" onError={e=>{e.currentTarget.style.display='none';const ph=e.currentTarget.nextSibling;if(ph&&ph.classList.contains('holocard-ph'))ph.style.display='block';}}/>:null}
        <div className="holocard-ph" style={src?{display:'none'}:(bgFallback?{}:{display:'none'})}/>
...
html,body{background:#020208;color:#f8f8ff;font-family:'Plus Jakarta Sans',sans-serif;-webkit-font-smoothing:antialiased;overflow-x:hidden;}
button,input,select,textarea{font-family:'Plus Jakarta Sans',sans-serif;font-size:16px;touch-action:manipulation;}
...
.srch-in{flex:1;min-width:0;padding:12px 16px;background:rgba(255,255,255,.05);
  border:1px solid rgba(255,255,255,.12);border-radius:14px;color:#f8f8ff;font-size:16px;
...
.feat:hover .feat-holo{opacity:0;}/* rainbow spin disabled */
...
.kcard:hover .holo-s{opacity:0;}/* rainbow spin disabled — keep clean shine only */
...
.dmod{background:linear-gradient(155deg,var(--s2) 0%,var(--s1) 100%);border:1px solid rgba(255,255,255,.1);
  ...
  animation:slideup .3s cubic-bezier(.34,1.56,.64,1);max-height:92svh;overflow-y:auto;
...
.dmod-img-wrap{width:100%;max-width:240px;margin:0 auto;flex-shrink:0;animation:cardpop .45s cubic-bezier(.22,1,.36,1);}
...
.paid-in{width:100%;padding:10px 14px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:9px;color:#f8f8ff;font-size:16px;outline:none;margin-bottom:10px;transition:border-color .2s;}
...
.smod{background:linear-gradient(155deg,var(--s2) 0%,var(--s1) 100%);border:1px solid rgba(255,255,255,.1);border-radius:20px 20px 0 0;padding:24px;padding-bottom:max(24px,env(safe-area-inset-bottom));width:100%;max-width:100%;animation:slideup .3s cubic-bezier(.34,1.56,.64,1);box-shadow:0 -20px 50px rgba(0,0,0,.6);}
...
.smod-in{width:100%;padding:11px 14px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:9px;color:#f8f8ff;font-size:16px;outline:none;margin-bottom:8px;transition:border-color .2s;}
...
/* PLANS MODAL */
.plans-modal{background:linear-gradient(155deg,var(--s2) 0%,var(--s1) 100%);border:1px solid rgba(255,255,255,.1);border-radius:20px 20px 0 0;padding:24px;padding-bottom:max(24px,env(safe-area-inset-bottom));width:100%;max-width:100%;animation:slideup .3s cubic-bezier(.34,1.56,.64,1);max-height:90svh;overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;box-shadow:0 -20px 50px rgba(0,0,0,.6);}
...
@media(min-width:640px){
  ...
  .srch-in{padding:14px 20px;font-size:16px;}
  ...
  .dmod-top{flex-direction:row;padding:20px;gap:22px;align-items:center;}
  .dmod-img{width:150px;max-width:150px;margin:0;}
  .dmod-img-wrap{width:250px;max-width:250px;margin:0;}
...
@media(min-width:1024px){
  ...
  .dmod{border-radius:26px;max-width:620px;animation:pop .22s cubic-bezier(.34,1.56,.64,1);}
...
@media(max-width:390px){
  .hero{padding:28px var(--p) 22px;}
  .hero-tagline{font-size:clamp(28px,9vw,52px);}
  .hero-sub{font-size:13px;margin-bottom:16px;}
  .grid{grid-template-columns:repeat(2,1fr);gap:10px;}
  .srch{gap:4px;}
  .srch-go{padding:10px 14px;font-size:11px;}
}
@media(max-width:360px){:root{--p:12px;}.srch-in{padding:10px 12px;font-size:16px;}}
@media(max-width:360px){.srch-go{padding:8px 10px;font-size:11px;}}
...
export default function DraGold(){
  ...
  const [sparkData,setSparkData] = useState({}); // {card_id: number[]} — price_history per sparkRange
  const [sparkRange,setSparkRange] = useState("7d"); // "7d"|"30d" (free) | "90d"|"6m"|"reg" (pro)
  const [portHistory,setPortHistory] = useState([]); // [{d:"YYYY-MM-DD",t:number}] localStorage snapshots
  ...
  const isPro=false; // will be true when Stripe plans activated
  ...
  // Portfolio history: snapshot giornaliero reale in localStorage
  // Salva il totale di oggi, carica la storia per il grafico
  useEffect(()=>{
    try{
      const stored=JSON.parse(localStorage.getItem('dg_port_hist')||'[]');
      setPortHistory(stored);
    }catch{}
  },[]);
  useEffect(()=>{
    ...
    try{
      const stored=JSON.parse(localStorage.getItem('dg_port_hist')||'[]');
      const updated=[...stored.filter(x=>x.d!==today),{d:today,t:+totalVal.toFixed(2)}]
        .sort((a,b)=>a.d.localeCompare(b.d)).slice(-90);
      localStorage.setItem('dg_port_hist',JSON.stringify(updated));
      setPortHistory(updated);
    }catch{}
  },[totalVal]);

  const portData=useMemo(()=>{
    const map={"1d":1,"2d":2,"3d":3,"7d":7,"30d":30,"90d":90,"1y":365};
    const days=map[portRange]||30;
    const cutoff=new Date(Date.now()-days*24*3600*1000).toISOString().slice(0,10);
    const filtered=portHistory.filter(x=>x.d>=cutoff);
    // Onesta: niente linea finta. Servono almeno 2 giorni REALI registrati.
    if(filtered.length<2) return null;
    return filtered.map(x=>x.t);
  },[portHistory,portRange]);
  const portChg=portData&&portData.length>=2?portData[portData.length-1]-portData[0]:null;
  const portTracking=portHistory.length>0;
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        const wIds=wrows.map(r=>r.card_api_id).filter(Boolean);
        ...
        if(wIds.length>0){
          try{
            const {data:priceRows}=await supabase
              ...
              .in('card_id',wIds);
            if(Array.isArray(priceRows)) priceMap=Object.fromEntries(priceRows.map(r=>[r.card_id,+r.price_market||0]));
          }catch{}
        }
        const remote=wrows.map(r=>({
          ...
          img:r.image_url||null,
          tcgType:r.tcg,
          market:priceMap[r.card_api_id]||0,
  ...
  // Fetch price_history for sparklines in vault cards (range driven by sparkRange + tier)
  useEffect(()=>{
    if(!supabaseReady) return;
    if(!col||col.length===0) return;
    const ids=col.map(x=>x.id).filter(id=>typeof id==='string'&&id.length>0);
    ...
    (async()=>{
      try{
        // Compute 'since' from sparkRange
        const now=Date.now();
        let since;
        if(sparkRange==="30d")       since=new Date(now-30*24*3600*1000).toISOString();
        else if(sparkRange==="90d")  since=new Date(now-90*24*3600*1000).toISOString();
        else if(sparkRange==="6m")   since=new Date(now-182*24*3600*1000).toISOString();
        else if(sparkRange==="reg")  since=user?.created_at||new Date(now-365*24*3600*1000).toISOString();
        else                         since=new Date(now-7*24*3600*1000).toISOString(); // default 7d
        const {data,error}=await supabase
          .from('price_history')
        ...
        if(Object.keys(map).length>0) setSparkData(map); // replace so stale ranges don't bleed through
      }catch{}
    })();
    return()=>{cancelled=true;};
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const DetailModal=({card})=>{
    ...
    // eBay Live Prices — /api/ebay-search (Vercel, eBay Browse API) con fallback a Supabase edge fn
    useEffect(()=>{
      ...
      (async()=>{
        try{
          ...
          // Filtra set code interni (bw8, dp7…) non riconoscibili su eBay
          const safeSet=_isInternalSetCode(setName)?'':(setName||'');
          const buildQ=(tcgLabel)=>`${card.name}${numStr?' '+numStr:''}${safeSet?' '+safeSet:''} ${langStr?langStr+' ':''}${tcgLabel}`.replace(/\s+/g,' ').trim();
          const qMap={
            pokemon:buildQ('pokemon card'),
            mtg:`${card.name}${numStr?' '+numStr:''}${safeSet?' '+safeSet:''} magic the gathering`,
            ygo:`${card.name}${numStr?' '+numStr:''} yugioh`,
            onepiece:buildQ('one piece card'),
          };
          const q=qMap[cardTcg]||buildQ('card');
          const market=(country||'US').toUpperCase();
          // Prova /api/ebay-search (dati diretti da eBay Browse API)
          let items=[];
          const resp=await fetch(`/api/ebay-search?q=${encodeURIComponent(q)}&market=${market}&limit=5`).catch(()=>null);
          if(resp?.ok&&resp.headers.get('content-type')?.includes('application/json')){
            const d=await resp.json().catch(()=>null);
            items=d?.items||[];
          }
          // Fallback: Supabase edge fn (se /api non disponibile)
          if(!items.length){
            const ctr=market.toLowerCase();
            const{data,error}=await supabase.functions.invoke('fetch-ebay-prices',{body:{query:q,country:ctr,limit:5}});
            if(!error) items=data?.items||[];
          }
          if(cancelled) return;
    ...
    const fetchPsaGrade=async(grade)=>{
      ...
      try{
        const market=(country||'US').toUpperCase();
        const q=`${card.name} PSA ${grade} ${cardTcg==='pokemon'?'pokemon':cardTcg==='mtg'?'magic gathering':cardTcg} graded card`;
        let items=[];
        const resp=await fetch(`/api/ebay-search?q=${encodeURIComponent(q)}&market=${market}&limit=5`).catch(()=>null);
        if(resp?.ok&&resp.headers.get('content-type')?.includes('application/json')){
          const d=await resp.json().catch(()=>null);
        ...
        if(grade===10) setEbayPsa10(items);
        else setEbayPsa9(items);
          ...
          {colTab==="vault"&&(<>
            {col.length===0?(
              ...
              <div className="bento">
                <div className="bento-main">
                  ...
                  <div className="range-row">
                    {["1d","7d","30d"].map(r=><button key={r} className={`rbtn${portRange===r?" on":""}`} onClick={()=>setPortRange(r)}>{r}</button>)}
                    {["90d","1y"].map(r=><button key={r} className={`rbtn${portRange===r?" on":""}`} onClick={()=>isPro?setPortRange(r):setPlansOpen(true)}>{r}{!isPro&&" 🔒"}</button>)}
                  </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8,flexWrap:"wrap"}}>
                <span style={{fontSize:10,color:"var(--muted)",fontFamily:"'Space Mono',monospace",marginRight:2}}>Sparkline:</span>
                {["7d","30d"].map(r=>(
                  <button key={r} className={`rbtn${sparkRange===r?" on":""}`} onClick={()=>setSparkRange(r)}>{r}</button>
                ))}
                {["90d","6m","reg"].map(r=>(
                  <button key={r} className={`rbtn${sparkRange===r?" on":""}`}
                    onClick={()=>isPro?setSparkRange(r):setPlansOpen(true)}>
                    {r==="reg"?"∞ reg":r}{!isPro&&" 🔒"}
                  </button>
                ))}
              </div>
