...
// In-memory token cache (Vercel functions are warm-cached for a few minutes)
...
export default async function handler(req, res) {
  ...
  const { q, market = 'US', language = 'EN', limit = '5' } = req.query
  if (!q) return res.status(400).json({ error: 'Missing q parameter' })

  const mk = MARKETPLACE_MAP[market] || MARKETPLACE_MAP.US

  // Append language keyword to refine search
  const LANG_KW = {
    JP: 'japanese', DE: 'deutsch', FR: 'francais',
    IT: 'italiano', ES: 'espanol', PT: 'portugues',
    KO: 'korean', ZH: 'chinese',
  }
  const langKw = LANG_KW[language] || ''
  const fullQuery = langKw ? `${q} ${langKw}` : q
  ...
  try {
    ...
    url.searchParams.set('filter', 'buyingOptions:{FIXED_PRICE},priceCurrency:' + mk.currency)
    if (market !== 'US' && market !== 'CA') {
      url.searchParams.set('filter', `${url.searchParams.get('filter')},itemLocationCountry:${market}`)
    }
