// scripts/cache-onepiece-images.js
// Priority 2 batch job: caches all One Piece card images into the
// DraGold-hosted card_image_cache (via api/cache-image.js), so the
// frontend's cached_url -> image_url -> placeholder priority chain
// actually has cache data to serve for One Piece.
//
// Usage: node scripts/cache-onepiece-images.js [--limit=N] [--dry-run]
//
...
async function fetchCardsToCache() {
  ...
  const cacheRows = cachePage.data || []
  const readySet = new Set(cacheRows.map(function (r) { return r.card_id }))
...
async function main() {
  console.log('===========================================')
  console.log('DraGold - Batch cache-image One Piece')
  console.log('===========================================')
  console.log('Avvio:', new Date().toISOString())

  let cards = await fetchCardsToCache()
  console.log('Carte One Piece da cachare: ' + cards.length)
  if (limit) {
